...
- 投稿なしのX read-only認証診断、actress候補1件だけの本番前preflight、連続実行を防ぐDB guard
- Scheduler CLIのPostgreSQL advisory lock、JST時刻・有効状態のDashboard表示（本番Schedulerは未有効）
...
本番1件の確認は、承認前にはpreflightだけを実行します。本文とmedia種別は表示しますが、affiliate URL全文は表示しません。

```powershell
npm run posts:live-one
```

実行には別途、確認token、`--execute`、`--confirm-one-post`、一時的な`DRY_RUN=false`がすべて必要です。現時点では実行しません。
