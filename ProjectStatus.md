...
Sprint 0で開発ワークフローを見直し、`AGENTS.md` に完走を優先する自己見積もり、実装順序、中断、コミットのルールを追加した。Step 3A（指定女優データ基盤）とStep 3B（指定女優管理API）は実装済みであり、次はStep 3C（指定女優管理画面）である。既存のYahoo!メール監視、JSON状態保存、投稿処理は変更していない。
...
Step 3C候補: 指定女優管理APIを利用し、追加・編集・有効化・無効化・削除を行う日本語の管理画面を実装する。
