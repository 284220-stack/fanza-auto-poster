...
Chrome拡張は`chrome-extension`ディレクトリをデベロッパーモードで「パッケージ化されていない拡張機能」として読み込む。正式Dashboard `https://fanza-auto-poster-production.up.railway.app`を同じブラウザーで一度開いてBasic認証を済ませ、FANZAのお気に入りページを表示して拡張popupを開く。Dashboard originは正式URLが読み取り専用で表示されるため通常は変更しない。origin、認証値、お気に入りURLは拡張へ保存されない。
...
2. 同じブラウザーでDashboardのBasic認証を済ませ、拡張の「FANZAセール一覧を開く」で固定URL`https://video.dmm.co.jp/av/list/`を開く。この操作だけでは抽出・API送信・persistは行われない。
3. 年齢確認が表示された場合は利用者が手動で完了し、一覧の読込みを待つ。別host/path、ログイン、エラー、読込途中ではcheck-onlyしない。
4. 拡張popupに正式Dashboard originが読み取り専用で表示されることを確認し、「セール商品を抽出してcheck-only」を一回実行する。
5. 抽出20件以内、上限超過0、不正0、API未掲載0、ID不一致0、metadata不完全0、server側VR除外0、失敗0、`snapshotComplete=true`、`schemaReady=true`と、sale掲載保存候補・favorite_sale候補・persist可否理由を確認する。
6. 全条件が一致した場合だけ、同じpopupの「確認済み内容をpersist」を一回押す。persist直前の再抽出または一回限りtoken検証に失敗した場合は再check-onlyし、再送しない。
7. Dashboardで現在セール掲載、取得経路、最終同期を確認し、dry-run previewでsaleとfavorite_saleの件数を確認する。
...
CLIはPostgreSQL advisory lockを持つが、複数Scheduler resourceは作らない。実行時刻は`SCHEDULER_TIME_JST=HH:mm`として運用者が決定し、承認までは空欄、`SCHEDULER_ENABLED=false`、`DRY_RUN=true`を維持する。

## X read-only診断

Dashboardの「同期・実行」からX接続確認を押す。処理は`v2.me()`だけで、投稿・media uploadを行わない。認証可否と不可逆なアカウント参照値だけを扱い、handle、ID、資格情報をログやProjectStatusへ記録しない。書込み権限、media upload可否、API plan/rate limitは実投稿前確認事項として残す。
...
Scheduler resourceを作らず、次を一回実行する。

```powershell
npm run posts:run
```

PostgreSQL advisory lock、最大5件、カテゴリ配分、30日、pending、media、本文を通る。`DRY_RUN=true`ではX API・履歴更新なしである。`--execute`は承認前に使用しない。live実行は有効なJST時刻、`SCHEDULER_ENABLED=true`、`DRY_RUN=false`が揃わない限り拒否される。

live開始時は`settings`にJST当日の実行guardを先に予約する。同日2回目は`scheduler_already_ran_today`で停止し、成功・失敗を理由に連続再実行しない。actress枠は女優ごとの最低投稿間隔と直近7日週間上限もDB履歴から確認する。
