...
5. FANZAのお気に入りページ、または指定セール一覧ページを開き、拡張popupへDashboardのoriginだけを入力する。
...
- セール抽出はHTTPSの`video.dmm.co.jp/av/list/`だけで許可する。別host・類似pathでは実行しない。
- ページHTML、FANZAのCookie、認証情報、localStorage、商品名を送信しない。
- 送信bodyは最大20件の正規化済み公式商品URLと制御値だけである。セール同期では完全集合フラグとcheck-only集合のSHA-256も送る。
