...
5. 拡張popupに正式production URLが読み取り専用で表示されることを確認する。通常運用では変更しない。
...
Dashboard originは`https://fanza-auto-poster-production.up.railway.app`へ固定され、拡張の再読み込み後も未保存状態のまま利用できます。Basic認証値を拡張へ入力・保存せず、事前認証済みの正式originへだけ要求します。401の場合は処理を停止します。

ローカル開発などで別originが必要な場合だけ「開発用詳細設定」を開き、そのpopupを開いている間だけ編集を有効にします。変更値は保存されず、編集を終了するか拡張を再読み込みすると正式production URLへ戻ります。
