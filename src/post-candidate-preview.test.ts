...
const candidate = { productId: 1, category: 'sale' as const, title: '【30%OFF】架空作品', actressNames: [], affiliateUrl: 'https://example.com/a', thumbnailUrl: 'https://pics.dmm.co.jp/image.jpg', selectionReasons: [], priorityScore: 1 };
...
const result = await new PostCandidatePreviewService(async () => selection, orchestrator, media).preview({ client: { createPost: async () => { throw new Error(); }, createReply: async () => { throw new Error(); } } });
...
assert.equal(result.items[0].mediaType, 'image');
