...
assert.equal(result.validUrlCount, 6);
assert.equal(result.invalidUrlCount, 1);
assert.equal(result.uniqueContentIdCount, 5);
assert.equal(result.metadataAvailableCount, 2);
assert.equal(result.metadataUnavailableCount, 1);
...
assert.deepEqual(calls.map((value) => value.id), ['one', 'two', 'missing', 'failed', 'vr']);
...
calls.length = 0;
const pageThree = await new FavoriteProductProvider(urls, metadata).fetch({ limit: 2, page: 3 });
