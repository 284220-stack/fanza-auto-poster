...
export type ProviderItem = {
  ...
  price?: number; salePrice?: number; isSale?: boolean; releaseDate?: string;
...
export type ProviderResult = { source: ProductSource; items: ProviderItem[]; fetchedAt: string; warnings: string[]; error?: string; nextPage?: number; hasMore: boolean };
...
function normalizeItem(item: ProviderItem): ProviderItem {
  ...
  if (price !== undefined && salePrice !== undefined && salePrice > price) throw new ProviderError('価格不正');
...
function number(value: number | undefined) { if (value === undefined) return undefined; if (!Number.isFinite(value) || value < 0) throw new ProviderError('価格が不正です。'); return value; }
