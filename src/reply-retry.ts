...
export class ReplyRetryService {
  ...
  async run(input: RetryReplyInput): Promise<RetryReplyResult> {
    ...
    try { try { const posted = await input.client.createReply(reply.reply.text, parent.xPostId); await this.history.create({ productId: input.productId, xPostId: posted.postId, postType: 'reply', executionStatus: 'posted', parentHistoryId: parent.id, postText: reply.reply.text }); await this.history.markReplyCompleted(parent.id); return { status: 'success', productId: input.productId, parentPostId: parent.xPostId, replyPostId: posted.postId, warnings: [], errors: [] }; } catch { return { status: 'failed', productId: input.productId, parentPostId: parent.xPostId, warnings: [], errors: ['reply_post_failed'] }; } } finally { this.running.delete(input.productId); }
  }
}
