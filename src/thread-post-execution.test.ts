...
let calls: string[] = [];
const client: XPostClient = { createPost: async (_text, media) => { calls.push(`parent:${media?.kind ?? 'none'}`); return { postId: 'parent-1', textLength: 2, createdAt: 'now' }; }, createReply: async (_text, id) => { calls.push(`reply:${id}`); return { postId: 'reply-1', textLength: 2, createdAt: 'now' }; } };
const service = new ThreadPostExecutionService();
const success = await service.run({ parentPostText: '【PR】\n本文', affiliateUrl: validUrl, productId: 'safe-id', media: { url: 'https://pics.dmm.co.jp/image.jpg', kind: 'image' }, client });
assert.equal(success.status, 'success'); assert.deepEqual(calls, ['parent:image', 'reply:parent-1']);
calls = [];
...
assert.equal((await service.run({ parentPostText: 'PR\n本文', affiliateUrl: validUrl, productId: 'safe-id', client })).status, 'failed');
assert.equal((await service.run({ parentPostText: `【PR】\n${validUrl}`, affiliateUrl: validUrl, productId: 'safe-id', client })).status, 'failed');
