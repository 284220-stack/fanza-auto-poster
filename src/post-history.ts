...
export class PostHistoryRepository {
  ...
  async hasWithin(productId: number, since: Date) { return (await this.db.query<{ exists: boolean }>('SELECT EXISTS(SELECT 1 FROM post_history WHERE product_id = $1 AND posted_at >= $2 AND execution_status = $3) AS exists', [productId, since.toISOString(), 'posted'])).rows[0]?.exists ?? false; }
