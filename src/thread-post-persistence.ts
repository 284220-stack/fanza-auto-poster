...
export class ThreadPostPersistenceService {
  ...
  async run(input: PersistedThreadPostInput): Promise<PersistedThreadPostResult> {
    ...
    try {
      ...
      let parentHistory; try { parentHistory = await this.history.create({ productId: input.productId, xPostId: parent.postId, postType: 'parent', executionStatus: 'pending_reply' }); } catch { return done({ status: 'partial_success', productId: input.productId, parentPostId: parent.postId, parentHistorySaved: false, replyHistorySaved: false, retryReplyPossible: true, warnings: [], errors: ['parent_history_failed'] }); }
      try { const posted = await input.client.createReply(reply.reply.text, parent.postId); await this.history.create({ productId: input.productId, xPostId: posted.postId, postType: 'reply', executionStatus: 'posted', parentHistoryId: parentHistory.id }); await this.history.markReplyCompleted(parentHistory.id); return done({ status: 'success', productId: input.productId, parentPostId: parent.postId, replyPostId: posted.postId, parentHistorySaved: true, replyHistorySaved: true, retryReplyPossible: false, warnings: [], errors: [] }); }
