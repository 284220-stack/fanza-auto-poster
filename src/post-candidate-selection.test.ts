...
assert.ok(withSchemaQueries[1].includes('minimum_post_interval_hours'));
assert.ok(withSchemaQueries[1].includes('weekly_post_limit'));
const disabledActressSale = await new PostCandidateSelectionService({ listSelectable: async () => [{ ...base, productId: 18, actressNames: ['無効女優'], enabledActressNames: [], enabledNewReleaseActressNames: [] }] }).select({ saleLimit: 1, actressLimit: 1, favoriteSaleLimit: 0 });
assert.equal(disabledActressSale.saleCandidates.length, 1);
assert.equal(disabledActressSale.actressCandidates.length, 0);
